...
lmux_app *lmux_app_new(const char *socket_path) {
    ...
    if (a->restore_on_start && a->auto_save_path[0]) {
        struct stat st;
        if (stat(a->auto_save_path, &st) == 0) {
            lmux_snapshot_load(a, a->auto_save_path);
        }
    }
...
char *lmux_browser_eval_js(lmux_pane *p, const char *script) {
    ...
    return strdup("{\"ok\":false,\"error\":\"no browser backend in headless build\"}");
}
...
#define RESULT_GROW do { \
    ...
    if (strcmp(cmd, "surface.split") == 0) {
        ...
        lmux_pane *p = lmux_pane_split(ws, s, dir, command[0] ? command : NULL);
        written = snprintf(result, result_cap,
    ...
    if (strcmp(cmd, "agent.spawn") == 0) {
        ...
        lmux_pane *p = lmux_pane_split(ws, s, LMUX_SPLIT_HORIZONTAL, command);
        if (!p) {
            written = snprintf(result, result_cap,
...
char *lmux_dispatch_json(lmux_app *app, const char *json_request) {
    if (!app || !json_request) {
        return strdup("{\"ok\":false,\"error\":{\"code\":\"invalid_request\",\"message\":\"null request\"}}");
    }
    ...
    if (!cmd[0]) {
        return strdup("{\"ok\":false,\"error\":{\"code\":\"invalid_request\",\"message\":\"missing cmd field\"}}");
    }
    ...
    if (!args_buf) {
        return strdup("{\"ok\":false,\"error\":{\"code\":\"oom\",\"message\":\"out of memory\"}}");
    }
    if (args_start) {
        ...
        for (const char *p = args_start; *p && j < args_cap - 1; p++) {
            if (j + 2 >= args_cap) {
                ...
                if (!tmp) { free(args_buf); return strdup("{\"ok\":false,\"error\":{\"code\":\"oom\",\"message\":\"out of memory\"}}"); }
                args_buf = tmp;
            }
            if (in_string) {
                args_buf[j++] = *p;
...
bool lmux_snapshot_load(lmux_app *app, const char *path) {
    ...
    if (!ws_arr) return true; /* empty save, fine */
    ws_arr += 14;
